package mods.scourgecraft.entities;
...
import mods.scourgecraft.PacketHandler;
...
public class TileEntityIronChest extends TileEntity implements IInventory {
    ...
    protected TileEntityIronChest(ChestType copper)
    {
        super();
        this.type = copper;
    ...
    public void updateEntity()
    {
        ...
        if (worldObj != null && !this.worldObj.isRemote && this.numUsingPlayers != 0 && (this.ticksSinceSync + this.xCoord + this.yCoord + this.zCoord) % 200 == 0)
        {
            ...
            while (var3.hasNext())
            {
                ...
                if (var4.openContainer instanceof ContainerChestBase)
                {
        ...
        {
            worldObj.addBlockEvent(xCoord, yCoord, zCoord, ScourgeCraftCore.chestBlockId, 3, ((numUsingPlayers << 3) & 0xF8) | (facing & 0x7));
        }
    ...
    public void openChest()
    {
        ...
        numUsingPlayers++;
        worldObj.addBlockEvent(xCoord, yCoord, zCoord, ScourgeCraftCore.chestBlockId, 1, numUsingPlayers);
    }
    ...
    public TileEntityIronChest applyUpgradeItem(ItemChestChanger itemChestChanger)
    {
        ...
        TileEntityIronChest newEntity = ChestType.makeEntity(itemChestChanger.getTargetChestOrdinal(getType().ordinal()));
        ...
        ScourgeBlockChest block = ScourgeCraftCore.chestBlock;
    ...
    public TileEntityIronChest updateFromMetadata(int l)
    {
        if (worldObj != null && worldObj.isRemote)
        {
            if (l != type.ordinal())
            {
                worldObj.setBlockTileEntity(xCoord, yCoord, zCoord, ChestType.makeEntity(l));
    ...
    public void handlePacketData(int typeData, int[] intData)
    {
        ...
        if (ChestType.values()[typeData].isTransparent() && intData != null)
        {
            int pos = 0;
    ...
    public void rotateAround(ForgeDirection axis)
    {
        ...
        worldObj.addBlockEvent(xCoord, yCoord, zCoord, ScourgeCraftCore.chestBlockId, 2, getFacing());
    }
}